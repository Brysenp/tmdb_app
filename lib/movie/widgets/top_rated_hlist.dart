...
import 'package:net_flix/movie/controller/top_rated_state_notifier.dart';
import 'package:net_flix/movie/widgets/horizontal_movie_skeletal.dart';

class TopRatedHList extends ConsumerWidget {
  const TopRatedHList({super.key, this.title = 'Top Rated'});
  final String? title;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(topRatedNotifierProvider);
    if (state == const GetMovieState.initial()) {
      WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
        ref.read(topRatedNotifierProvider.notifier).getTopRatedMovies();
      });
    }
    return Column(
